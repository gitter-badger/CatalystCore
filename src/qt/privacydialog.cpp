...
void PrivacyDialog::on_pushButtonZCondominiumControl_clicked()
{
    ZCondominiumControlDialog* zCDMControl = new ZCondominiumControlDialog(this);
    zCDMControl->setModel(walletModel);
    zCDMControl->exec();
}

void PrivacyDialog::setZCondominiumControlLabels(int64_t nAmount, int nQuantity)
{
...
void PrivacyDialog::sendzCDM()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Catalyst Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CATALYST </b>will be added to the standard transaction fees!<br />";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Catalyst, ";
