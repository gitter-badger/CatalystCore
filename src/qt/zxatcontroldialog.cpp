...
#include "zxatcontroldialog.h"
#include "ui_zxatcontroldialog.h"
...
std::list<std::string> ZCondominiumControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZCondominiumControlDialog::listMints;

ZCondominiumControlDialog::ZCondominiumControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZCondominiumControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZCondominiumControlDialog::~ZCondominiumControlDialog()
{
    delete ui;
}

void ZCondominiumControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCondominiumControlDialog::updateList()
{
...
void ZCondominiumControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCondominiumControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZCondominium_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCondominiumControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZCondominiumControlDialog::GetSelectedMints()
{
...
void ZCondominiumControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
